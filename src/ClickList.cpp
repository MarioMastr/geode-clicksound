...
static std::string getmodfix() { return ""_spr; }
...
const char *Clicks::getClickSprite(int clickSliderValue)
{
    ...
    switch (clickSliderValue) {
        ...
        case 20:
            clickSoundInUse = convertToConstChar(fmt::format("{}Viper_Clicks_SpaceBar_{}.ogg", getmodfix(), std::to_string((rand() % 5) + 1)));
            break;
        case 21:
            clickSoundInUse =
                convertToConstChar(fmt::format("{}logitech_mx_anywhere_2s_Clicks_{}.ogg", getmodfix(), std::to_string((rand() % 5) + 1)));
            break;
        ...
        case 31:
            clickSoundInUse = convertToConstChar(fmt::format("{}Logitech_G502-{}-{}.ogg", getmodfix(),
                                                             (rand() % 100 == 0) ? "soft" : ((rand() % 100 > 50) ? "hard" : "normal"),
                                                             std::to_string((rand() % 10) + 1)));
            break;
        case 32:
            clickSoundInUse = convertToConstChar(fmt::format("{}up-arrow-4-click-{}.wav", getmodfix(), std::to_string((rand() % 10) + 1)));
            break;
        // Meme sounds
        ...
        case -18:
            clickSoundInUse = (rand() % 5 == 0)
                                  ? "dash-one.ogg"_spr
                                  : ((rand() % 5 == 1) ? "dash-two.ogg"_spr
                                                       : ((rand() % 5 == 2) ? "dash-three.ogg"_spr
                                                                            : ((rand() % 5 == 3) ? "dash-dash.ogg"_spr : "dash-geometry.ogg"_spr)));
            break;

...
const char *Clicks::getReleaseSprite(int clickSliderValue)
{
    ...
    switch (clickSliderValue) {
        ...
        case 9:
            releaseSoundInUse = convertToConstChar(fmt::format("{}Viper_SpaceBar_release_{}.ogg", getmodfix(), std::to_string((rand() % 3) + 1)));
            break;
        case 10:
            releaseSoundInUse =
                convertToConstChar(fmt::format("{}logitech_mx_anywhere_2s_Releases_{}.ogg", getmodfix(), std::to_string((rand() % 5) + 1)));
            break;
        ...
        case 16:
            releaseSoundInUse = convertToConstChar(fmt::format("{}up-arrow-4-release-{}.ogg", getmodfix(), std::to_string((rand() % 10) + 1)));
            break;

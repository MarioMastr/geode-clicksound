...
{
    void YOUAREGAY(CCObject *) { geode::openSettingsPopup(Mod::get()); }
    ...
    void customSetup()
    {
        ...
        auto btn = CCMenuItemSpriteExtra::create(spr, this, menu_selector(YoureAPansexual::YOUAREGAY));
        spr->setScale(0.7f);
...
class $modify(editor, EditorPauseLayer)
{
    ...
    bool init(LevelEditorLayer *po)
    {
        ...
        auto spr     = CCSprite::create("Button.png"_spr);
        spr->setScale(0.6f);
        menu->setLayout(RowLayout::create()->setGap(0.f)->setAxisAlignment(AxisAlignment::Center)->setAutoScale(false)->setCrossAxisOverflow(true));
        auto btn = CCMenuItemSpriteExtra::create(spr, this, menu_selector(editor::YOUAREGAY));
        if (Mod::get()->getSettingValue<bool>("settings-button")) {
            btn->setPosition({ menu->getContentSize().width / 2, btn->getContentSize().height / 2 });
            btn->setID("beat.hewwo!_:3");
            menu->addChild(btn);
